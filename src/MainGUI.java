...
public class MainGUI{
    ...
    //Lavendar, cyan,  yellow green, dark blue, light green, 
    private Color[] colorPaletteOptions = { new Color(166,77,255), new Color(0, 213, 255), new Color(115,230,0), new Color(0,34,204), new Color(0,128,0)};
    ...
    class IPHeavyTask extends SwingWorker<Integer,String>{
        ...
        public Integer doInBackground(){
        	// Pradeep. Update the height and width size here. 37 32.
            int imageSizeThres = 74;
            int imageSizeThresHeight = 64;
            Image toolkitImage = null;
            if (image.getHeight() > imageSizeThres && image.getWidth() > imageSizeThres) {
                toolkitImage=  image.getScaledInstance(imageSizeThres, imageSizeThresHeight, Image.SCALE_DEFAULT);
                image = new BufferedImage(imageSizeThres, imageSizeThresHeight, BufferedImage.TYPE_INT_ARGB);
                ...
                image = new BufferedImage(image.getWidth(), imageSizeThresHeight, BufferedImage.TYPE_INT_ARGB);
                Graphics g = image.getGraphics();
                g.drawImage(toolkitImage, 0, 0, null);
                g.dispose();
    ...
    private class RepaintDARPclasss implements ActionListener{
        public void actionPerformed(ActionEvent event){
        	...
        	// Changed by Pradeep to fix the robot colors.
            for (int r=0;r<nr;r++){ColorsNr[r]=colorPaletteOptions[r];}
            DarpResult.paint();
            mCPPResult.paint();
            mainFrame.setVisible(true);
            mainFrame.repaint();
